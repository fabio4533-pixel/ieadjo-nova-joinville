...
async function arrange(){
 ...
 const pagehead=view?.querySelector('.pagehead');
 ...
 if(!box){
   ...
   songMap=songs||[];
   const countBy={};songMap.forEach(s=>{const k=String(s.schedule_id);countBy[k]=(countBy[k]||0)+1});
   const used=(schedules||[]).filter(s=>countBy[String(s.id)]>0);
   box=document.createElement('section');box.id='leaderCultosFirst';box.style.cssText='margin:0 0 10px;padding:14px;border:1px solid #c8d9e8;border-radius:20px;background:linear-gradient(145deg,#eef7ff,#ffffff);box-shadow:0 10px 28px #0e2a4716';
   box.innerHTML=`<div style="display:flex;align-items:center;gap:8px;margin:0 0 10px"><span style="width:32px;height:32px;border-radius:10px;background:#174f7c;color:#fff;display:grid;place-items:center;font-size:17px">⛪</span><div><div style="font-size:11px;font-weight:900;letter-spacing:.9px;color:#174f7c">CULTOS</div><div style="font-size:12px;color:#6b7885">Selecione um culto para ver os hinos</div></div></div>${used.length?used.map(g=>`<button type="button" data-leader-culto="${g.id}" style="width:100%;border:1px solid #d3e1ec;background:#fff;border-radius:15px;padding:13px 14px;margin:0 0 8px;text-align:left;display:flex;align-items:center;justify-content:space-between;gap:10px;box-shadow:0 4px 12px #0e2a470a"><span><b style="display:block;color:#0e2a47;font-size:15px">${esc(g.title)}</b><small style="color:#687887">${esc(fmt(g.event_date))}${g.time_text?' • '+esc(g.time_text):''} • ${countBy[String(g.id)]} hino${countBy[String(g.id)]===1?'':'s'}</small></span><span style="font-size:20px;color:#174f7c">›</span></button>`).join(''):'<div class="card"><div class="grow"><b>Nenhum culto com hinos selecionados.</b></div></div>'}`;
   pagehead.insertAdjacentElement('afterend',box);
   box.querySelectorAll('[data-leader-culto]').forEach(b=>b.onclick=async()=>{currentCulto=String(b.dataset.leaderCulto);box.querySelectorAll('[data-leader-culto]').forEach(x=>{const on=String(x.dataset.leaderCulto)===currentCulto;x.style.borderColor=on?'#174f7c':'#d3e1ec';x.style.background=on?'#eaf4fc':'#fff';x.style.boxShadow=on?'0 7px 18px #174f7c22':'0 4px 12px #0e2a470a'});if(!songMap.length)songMap=await api('/rest/v1/ieadjo_songs?select=id,schedule_id&schedule_id=not.is.null');filterSongs(songMap)});
 }
 if(pagehead.nextElementSibling!==box)pagehead.parentNode.insertBefore(box,pagehead.nextSibling);
 const search=view.querySelector('[data-rep-search]');
 if(search&&box.nextElementSibling!==search)box.insertAdjacentElement('afterend',search);
}
...
setInterval(()=>arrange().catch(()=>{}),500);arrange().catch(()=>{});
})();