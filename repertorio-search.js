...
function placeAtTop(container,wrap,mode){
  if(mode==='app'){
    const head=container.querySelector('.pagehead');
    const manager=container.querySelector('#weeklyRepManager');
    const firstSong=container.querySelector('.dynamic-song,.dynamic-empty,.song');
    const anchor=manager||firstSong;
    if(anchor){container.insertBefore(wrap,anchor)}
    else if(head){head.insertAdjacentElement('afterend',wrap)}
  }else{
    const h=container.querySelector('h3');
    const firstControls=container.querySelector('#importSongsBtn')?.parentElement||container.querySelector('#editingSongId')?.parentElement||container.querySelector('#items');
    if(firstControls){container.insertBefore(wrap,firstControls)}
    else if(h){h.insertAdjacentElement('afterend',wrap)}
  }
}
function makeSearch(container,mode){
  let wrap=container.querySelector('[data-rep-search]');
  if(!wrap){
    wrap=document.createElement('div');wrap.setAttribute('data-rep-search','1');wrap.style.cssText='margin:10px 0 14px;position:relative';
...
function observe(){scanApp();scanAdmin();const targets=[document.getElementById('view'),document.getElementById('work')].filter(Boolean);targets.forEach(t=>new MutationObserver(()=>{scanApp();scanAdmin()}).observe(t,{childList:true,subtree:true}));if(!targets.length)setTimeout(observe,150)}
observe();
})();