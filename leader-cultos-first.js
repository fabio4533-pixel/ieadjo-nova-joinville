...
async function arrange(){
 ...
 if(!box){
   ...
   box.querySelectorAll('[data-leader-culto]').forEach(b=>b.onclick=async()=>{currentCulto=String(b.dataset.leaderCulto);box.querySelectorAll('[data-leader-culto]').forEach(x=>{const on=String(x.dataset.leaderCulto)===currentCulto;x.style.borderColor=on?'#174f7c':'#dbe4ec';x.style.background=on?'#eef6fc':'#fff'});if(!songMap.length)songMap=await api('/rest/v1/ieadjo_songs?select=id,schedule_id&schedule_id=not.is.null');filterSongs(songMap)});
 }
 // Mantém os cultos fisicamente presos logo abaixo do título, acima de pesquisa, gerenciamento e hinos.
