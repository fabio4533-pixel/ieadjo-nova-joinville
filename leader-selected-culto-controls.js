...
async function render(scheduleId,anchor){
 ...
 box.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin:0 0 8px"><div style="font-size:11px;font-weight:900;letter-spacing:.8px;color:#6b7885">HINOS SELECIONADOS</div><small style="color:#6c7a89">${songs.length} hino${songs.length===1?'':'s'}</small></div>${songs.length?songs.map((s,i)=>`<div class="card selected-culto-song" style="padding:12px;margin-bottom:8px"><div style="display:flex;align-items:flex-start;gap:10px"><div style="width:30px;height:30px;border-radius:9px;background:#174f7c;color:#fff;display:grid;place-items:center;font-weight:900;flex:none">${i+1}</div><div class="grow"><b>${esc(s.title)}</b>${s.artist?`<span>${esc(s.artist)}</span>`:''}<span>Tom: ${esc(s.song_key||'-')}</span><div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:9px"><button type="button" data-selected-up="${s.id}" ${i===0?'disabled':''} style="border:0;border-radius:8px;background:#eef4f9;color:#0e2a47;padding:8px 10px;font-size:10px;font-weight:900;${i===0?'opacity:.4;':''}">↑ Subir</button><button type="button" data-selected-down="${s.id}" ${i===songs.length-1?'disabled':''} style="border:0;border-radius:8px;background:#eef4f9;color:#0e2a47;padding:8px 10px;font-size:10px;font-weight:900;${i===songs.length-1?'opacity:.4;':''}">↓ Descer</button><button type="button" data-selected-remove="${s.id}" style="border:0;border-radius:8px;background:#fff4df;color:#8a5c09;padding:8px 10px;font-size:10px;font-weight:900">Remover do culto</button><button type="button" data-selected-delete="${s.id}" style="border:0;border-radius:8px;background:#fff0f0;color:#a83232;padding:8px 10px;font-size:10px;font-weight:900">🗑 Apagar hino</button></div></div></div></div>`).join(''):'<div class="card"><div class="grow"><b>Nenhum hino selecionado neste culto.</b></div></div>'}`;
 box.onclick=async e=>{
   const up=e.target.closest?.('[data-selected-up]'),down=e.target.closest?.('[data-selected-down]'),remove=e.target.closest?.('[data-selected-remove]'),del=e.target.closest?.('[data-selected-delete]');
   if(working||(!up&&!down&&!remove&&!del))return;working=true;
   try{
     if(del){if(!confirm('Apagar este hino definitivamente da biblioteca?'))return;await api('/rest/v1/ieadjo_songs?id=eq.'+encodeURIComponent(del.dataset.selectedDelete),{method:'DELETE',headers:{Prefer:'return=minimal'}})}
     else if(remove){if(!confirm('Remover este hino deste culto? Ele continuará salvo na biblioteca.'))return;await api('/rest/v1/ieadjo_songs?id=eq.'+encodeURIComponent(remove.dataset.selectedRemove),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({schedule_id:null})})}
